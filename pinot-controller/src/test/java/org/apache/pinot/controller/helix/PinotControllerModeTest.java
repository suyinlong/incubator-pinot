...
public class PinotControllerModeTest extends ControllerTest {
  ...
  public void testHelixOnlyController()
      throws Exception {
    config.setControllerMode(HelixSetupUtils.ControllerMode.HELIX_ONLY);
    ...
    Assert.assertEquals(_controllerStarter.getControllerMode(), HelixSetupUtils.ControllerMode.HELIX_ONLY);
  ...
  public void testDualModeController()
      throws Exception {
    config.setControllerMode(HelixSetupUtils.ControllerMode.DUAL);
    config.setControllerPort(Integer.toString(Integer.parseInt(config.getControllerPort()) + controllerPortOffset++));

    startController(config);
    TestUtils.waitForCondition(aVoid -> _helixManager.isConnected(), TIMEOUT_IN_MS,
        "Failed to start " + config.getControllerMode() + " controller in " + TIMEOUT_IN_MS + "ms.");
    Assert.assertEquals(_controllerStarter.getControllerMode(), HelixSetupUtils.ControllerMode.DUAL);
  ...
  public void testPinotOnlyController()
    ...
    config.setControllerMode(HelixSetupUtils.ControllerMode.PINOT_ONLY);
    config.setControllerPort(Integer.toString(Integer.parseInt(config.getControllerPort()) + controllerPortOffset++));
    ...
    config2.setControllerMode(HelixSetupUtils.ControllerMode.HELIX_ONLY);
    ...
    TestUtils.waitForCondition(aVoid -> _helixResourceManager.getHelixZkManager().isConnected(), TIMEOUT_IN_MS,
        "Failed to start " + config.getControllerMode() + " controller in " + TIMEOUT_IN_MS + "ms.");
    Assert.assertEquals(_controllerStarter.getControllerMode(), HelixSetupUtils.ControllerMode.PINOT_ONLY);
